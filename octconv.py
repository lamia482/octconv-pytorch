...
class OctConv(torch.nn.Module):
  def __init__(self, ch_in, ch_out, kernel_size, 
    ...
    if alphas is None: alphas = (0.2, 0.5)
    ...
    self.X2L = Conv2d(ch_in, self.lf_in, kernel_size, stride, padding, dilation, groups, bias)
    self.X2H = Conv2d(ch_in, self.hf_in, kernel_size, stride, padding, dilation, groups, bias)
    ...
    self.L2X = Conv2d(self.lf_out, ch_out, kernel_size, stride, padding, dilation, groups, bias)
    self.H2X = Conv2d(self.hf_out, ch_out, kernel_size, stride, padding, dilation, groups, bias)
  ...
  def forward_beta(self, x):
    print('=' * 10 + '  >>> target on x: {}'.format(x.shape))
    ...
    HtoH = self.H2H(x_hf)
    print('L2L: {}, L2H: {}, H2L: {}, H2H:{}'.format(LtoL.shape, LtoH.shape, HtoL.shape, HtoH.shape))
    L = LtoL + HtoL
    H = LtoH + HtoH
    output = self.upscale(self.L2X(L)) + self.H2X(H)
    print('*' * 10 + '  <<< output: {}'.format(output.shape))
    return output
